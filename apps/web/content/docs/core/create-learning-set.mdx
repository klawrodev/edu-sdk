...
`content` may be a string or [`FileContent`](/docs/core/extract-content) (PDF / text / markdown bytes). File content is extracted once before nested generators run.

When `learnerContext` is set, it is forwarded to every nested `create*` call. See [Personalization](/docs/guides/personalization).
...
See [Concepts](/docs/getting-started/concepts) for nested artifact shapes, and [Study session flow](/docs/guides/study-session-flow) when materials come from a session.
