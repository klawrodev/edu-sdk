...
Generated materials may include optional `topics`; quiz grades may include `byTopic`. This runner does not display those fields yet. Pass `learnerContext` at generation time — see [Personalization](/docs/guides/personalization).
...
For a fully custom runner (own timers / navigation), see [Study session flow](/docs/guides/study-session-flow). See [Styling](/docs/react/styling) for `styles.css` and CSS variables.
